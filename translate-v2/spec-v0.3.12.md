...
- 双向平衡（防矫枉过正到死板直译）：本铁律只砍"译文自己多加的赘余"，绝不取消
  §3.1 的二次创作授权——句法重组、语气再造、节奏设计、习语替换、地道化表达，只要
  有源文锚点、服务原文功能，该做就大胆做。走向另一极端——词序搬运、逐词贴字面、
  句法尸检、机器腔/翻译腔（只覆盖字典义、不像目标语成品）——同样是失真，照扣
  fluency_mechanics / voice_register / rhythm_prosody / reader_reachability。判据永远是
  "每个删/留/改是否服务原文功能与目标语成品感"，不是"改得多还是少、贴不贴字面"。
- 顺口豁免：某个被判"赘余"的语气助词/词，若删去后目标语读来实在不顺口（破坏语感、
  节奏、口气、停顿或成品感），允许保留不删——顺口/成品感优先于机械去赘。判据仍是
  目标语成品感与朗读流畅，不是"能删就删、删到最少"。
