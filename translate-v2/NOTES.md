...
- 小D 的译前上下文步骤不因本地成员能自读目录而省略：既有状态机（小D 从本地证据起草
  译前上下文包 -> conductor 校验 -> 用户审阅 -> 才进 formal relay）不变；梳理产物同时
  ① 给全体成员一份统一口径的 briefing、免各自重读，② 服务小克这类无本地权限成员。
- 项目目录是背景理解，不是待译源文；守 source-boundary，不得把知识库 / 写作区内容倒灌
  进译文当额外内容（呼应本 NOTES「上下文只作选词依据、严禁倒灌进译文」Addition 红线）。
